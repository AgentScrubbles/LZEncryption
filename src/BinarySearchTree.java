...
public class BinarySearchTree implements ADT{
	...
	private int dictionaryIndexSize = 3;
	...
	public BinarySearchTree(String dictionaryBitString, int dictionaryIndexSize){
		this.dictionaryIndexSize = dictionaryIndexSize;
		initialize();
		buildDictionary(dictionaryBitString);
	}
	...
	public void buildDictionary(String bitString) {
		...
		for(int i = 0; i < bitString.length(); i += dictionaryIndexSize){
			add(bitString.substring(i, dictionaryIndexSize));
		}
